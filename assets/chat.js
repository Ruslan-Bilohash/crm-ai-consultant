 ...
 * Version: 2.5.4 — Остаточна CSP Safe версія
 */
...
(function () {
    ...
    if (!window.crmAI || !window.crmAI.site_id) {
        console.error('CRM AI Consultant: crmAI не ініціалізовано');
        return;
    }
    ...
    console.log('✅ CRM AI Consultant: віджет повністю запущено для', s.site_id);
    ...
    chat.style.cssText = `
        ...
        background:#0f172a; border-radius:24px; box-shadow:0 20px 60px rgba(0,0,0,0.6);
        display:none; flex-direction:column; z-index:99999; overflow:hidden;
        border:1px solid rgba(148,163,184,0.3);
    `;

    chat.innerHTML = `
        <div style="background:linear-gradient(135deg,#1e2937,#334155);padding:18px 20px;color:#fff;display:flex;align-items:center;justify-content:space-between;">
        ...
        <div id="messages" style="flex:1;padding:20px;overflow-y:auto;background:rgba(15,23,42,0.95);"></div>
        <div style="padding:15px 20px;background:rgba(15,23,42,0.98);display:flex;gap:10px;">
            ...
            <button id="sendBtn" style="width:56px;height:56px;background:#22d3ee;border:none;border-radius:9999px;color:#000;font-size:24px;">→</button>
        </div>
    `;
    ...
    function addMessage(text, isUser) {
        ...
        div.style.cssText = isUser 
            ? 'margin-left:auto;background:#22d3ee;color:#000;padding:12px 18px;border-radius:18px 18px 4px 18px;max-width:80%;margin-bottom:10px;'
            : 'margin-right:auto;background:rgba(255,255,255,0.15);padding:12px 18px;border-radius:18px 18px 18px 4px;max-width:80%;margin-bottom:10px;';
    ...
    async function sendMessage() {
        ...
        try {
            await fetch(s.ajax_url, { method: 'POST', body: fd });
        } catch (e) {
    ...
    // Події
    openBtn.onclick = () => { chat.style.display = 'flex'; openBtn.style.display = 'none'; };
    document.getElementById('closeBtn').onclick = () => { chat.style.display = 'none'; openBtn.style.display = 'block'; };
